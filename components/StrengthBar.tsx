      ...
      {result.characterAnalysis.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span className="font-mono">{result.crackTimeCategory}</span>
          </p>
          <p
            className={`mt-1 ${mutedClasses} ${
              presenterMode ? 'text-sm' : 'text-xs'
            }`}
          >
            Rough estimate only. Actual time depends on hashing algorithm,
            rate limits, and attacker resources.
          </p>
        </div>
      )}
    </div>
